...
def generateJobConfigTemplate(reader, writer):
    ...
    readerTemplatePath = "%s/plugin/reader/%s/plugin_job_template.json" % (os.path.dirname(DATAX_HOME),reader)
    print("readerTemplatePath==="+readerTemplatePath)
    writerTemplatePath = "%s/plugin/writer/%s/plugin_job_template.json" % (os.path.dirname(DATAX_HOME),writer)
    try:
