...
public abstract class Channel {
  ...
  public Channel(final Configuration conf) {
    ...
    long byteSpeed = conf
        .getLong(CoreConstant.DATAX_CORE_TRANSPORT_CHANNEL_SPEED_BYTE, 1024 * 1024);
    ...
    synchronized (isFirstPrint) {
      if (isFirstPrint) {
        LOG.info("Channel set byte_speed_limit to " + byteSpeed + (byteSpeed <= 0 ? ","
            + " No bps activated." : "."));
        LOG.info("Channel set record_speed_limit to " + recordSpeed + (recordSpeed <= 0
            ? ", No tps activated." : "."));
        isFirstPrint = false;
      }
    }
    ...
    if (interval - this.flowControlInterval >= 0) {
      ...
      if (isChannelByteSpeedLimit) {
        ...
        long currentByteSpeed =
            (CommunicationTool.getTotalReadBytes(currentCommunication) - CommunicationTool
