 ...
 * 实例代码:<br>
 * <p/>
 * 获取job的配置信息<br> Configuration configuration = Configuration.from(new File("Config.json")); <br> String
 * jobContainerClass = configuration.getString("core.container.job.class"); <br>
 * <p/>
 * <br>
 * 设置多级List <br> configuration.set("job.reader.parameter.jdbcUrl", Arrays.asList(new String[] {"jdbc", "jdbc"}));
 * <p/>
 * <p/>
 * <br>
 * <br>
 ...
 * <br>
 * <p/>
 * Configuration 存在两种较好地实现方式<br> 第一种是将JSON配置信息中所有的Key全部打平，用a.b.c的级联方式作为Map的Key，内部使用一个Map保存信息 <br>
 * 第二种是将JSON的对象直接使用结构化树形结构保存<br>
 * <p/>
 * 目前使用的第二种实现方式，使用第一种的问题在于: <br> 1. 插入新对象，比较难处理，例如a.b.c="bazhen"，此时如果需要插入a="bazhen"，也即是根目录下第一层所有类型全部要废弃
 * ，使用"bazhen"作为value，第一种方式使用字符串表示key，难以处理这类问题。 <br> 2. 返回树形结构，例如 a.b.c.d = "bazhen"，如果返回"a"下的所有元素，实际上是一个Map，需要合并处理 <br>
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw DataXException
                    .asDataXException(CommonErrorCode.RUNTIME_ERROR, "Path不能为null，该异常代表系统编程错误, 请联系DataX开发团队 !");
        }
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR,
                    String.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系DataX开发团队! .", StringUtils.join(paths, "."),
                            path));
        }
