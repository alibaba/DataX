 ...
 * 实例代码:<br>
 * <p/>
 * 获取job的配置信息<br> Configuration configuration = Configuration.from(new File("Config.json")); <br>
 * String
 * jobContainerClass = configuration.getString("core.container.job.class"); <br>
 * <p/>
 * 设置多级List <br> configuration.set("job.reader.parameter.jdbcUrl", Arrays.asList(new String[]
 * {"jdbc", "jdbc"}));
 * <p/>
 * <br>
 * 合并Configuration: <br> configuration.merge(another);
 * <p/>
 * <br>
 * <p/>
 * Configuration 存在两种较好地实现方式<br> 第一种是将JSON配置信息中所有的Key全部打平，用a.b.c的级联方式作为Map的Key，
 * 内部使用一个Map保存信息 <br>
 * 第二种是将JSON的对象直接使用结构化树形结构保存<br>
 * <p/>
 * 目前使用的第二种实现方式，使用第一种的问题在于: <br> 1. 插入新对象，比较难处理，例如a.b.c="bazhen"，此时如果需要插
 * 入a="bazhen"，也即是根目录下第一层所有类型全部要废弃
 * ，使用"bazhen"作为value，第一种方式使用字符串表示key，难以处理这类问题。 <br> 2. 返回树形结构，例如 a.b.c.d =
 * "bazhen"，如果返回"a"下的所有元素，实际上是一个Map，需要合并处理 <br>
...
public class Configuration {
  ...
  public static Configuration from(File file) {
    try {
      ...
      throw DataXException.asDataXException(CommonErrorCode.CONFIG_ERROR,
          String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", file.getAbsolutePath(),
              e));
    }
  }
  ...
  public String getNecessaryValue(String key, ErrorCode errorCode) {
    ...
    if (StringUtils.isBlank(value)) {
      throw DataXException
          .asDataXException(errorCode, String.format("您提供配置文件有误，[%s]是必填参数，不允许为空或者留白 .", key));
    }
   ...
   * {"a": {"b": {"c": [0,1,2,3]}}}
   * <p/>
   * config.get("") 返回整个Map <br> config.get("a") 返回a下属整个Map <br> config.get("a.b.c") 返回c对应的数组List
   * <br>
   ...
   * <p/>
   * <br>
   * 对于插入对象，Configuration不做任何限制，但是请务必保证该对象是简单对象(包括Map<String, Object>、List<Object>)，不要使用自定义对象，否则后续对于JSON序列化等情况会出现未定义行为。
   *
  ...
  Object buildObject(final List<String> paths, final Object object) {
    if (null == paths) {
      throw DataXException
          .asDataXException(CommonErrorCode.RUNTIME_ERROR,
    ...
    for (int i = paths.size() - 1; i >= 0; i--) {
      ...
      throw DataXException.asDataXException(CommonErrorCode.RUNTIME_ERROR,
          String.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系DataX开发团队! .",
  ...
  @SuppressWarnings("unchecked")
  Object setObjectRecursive(Object current, final List<String> paths, int index,
      final Object value) {
    ...
    throw DataXException
        .asDataXException(CommonErrorCode.RUNTIME_ERROR, "该异常代表系统编程错误, 请联系DataX开发团队 !");
  }
  ...
  private void checkPath(final String path) {
    ...
    for (final String each : StringUtils.split(".")) {
      if (StringUtils.isBlank(each)) {
        throw new IllegalArgumentException(
            String.format("系统编程错误, 路径[%s]不合法, 路径层次之间不能出现空白字符 .", path));
      }
    }
  }
