...
import com.alibaba.datax.plugin.reader.obsreader.ObsInputStream;
 ...
 */
public class ObsSplitUtil extends UnstructuredSplitUtil {
    ...
    public ObsSplitUtil(ObsClient obsClient, String bucketName) {
        super(false);
    ...
    public Long getFileTotalLength(String filePath) {
        ...
        ObsObject obsObject = this.obsClient.getObject(getObjectRequest);
        return obsObject.getMetadata().getContentLength();
    }

    @Override
    public InputStream getFileInputStream(StartEndPair startEndPair) {
        InputStream inputStream = new ObsInputStream(this.obsClient, this.bucketName, startEndPair.getFilePath(),
                startEndPair.getStart(), startEndPair.getEnd());
        return inputStream;
    }

    private Boolean canSplitSingleFile(Configuration jobConfig) {
        Boolean enableInnerSplit = jobConfig.getBool(Key.ENABLE_INNER_SPLIT, true);
        if (!enableInnerSplit) {
            return false;
        }

        // 默认不切分
        String fileFormat = jobConfig.getString(Key.FILE_FORMAT,
                com.alibaba.datax.plugin.unstructuredstorage.reader.Constant.DEFAULT_FILE_FORMAT);
        String compressType = jobConfig.getString(Key.COMPRESS);
    ...
    private void reBalanceGroup(List<Group> groups) {
        ...
        FileBlock splitOutBlock = groupMaxLoad.split(splitLen, this.obsClient, this.bucketName);
    ...
    public List<Configuration> getSplitedConfigurations(Configuration originConfiguration, List<Pair<String, Long>> objKeySizePares,
        ...
        this.balanceThreshold = originConfiguration.getDouble(com.alibaba.datax.plugin.reader.obsreader.Key.BALANCE_THRESHOLD, 10.0);
        ...
        for (Group group : groups) {
            ...
            LOG.info("是否允许切分 : " + canSplitSingleFile(originConfiguration));
...
//                    startEndPairs.add(new StartEndPair(fileBlock.getStartOffset(), -1L, fileBlock.getObjName()));
                    LOG.info("StartOffset : " + fileBlock.getStartOffset() + "，EndOffset : " + fileBlock.getEndOffset());
                    startEndPairs.add(new StartEndPair(fileBlock.getStartOffset(), fileBlock.getEndOffset(), fileBlock.getObjName()));
                }
            }
        ...
        for (Group group : groups) {
            LOG.info("group2 = " + group.toString());
        }
...
class Group {
     ...
     * */
    FileBlock split(Long splitLen, ObsClient obsClient, String obsBucketName) {
        ...
        if (bigBlock.getSize() <= ObsSplitUtil.SINGLE_FILE_SPLIT_THRESHOLD_IN_SIZE) {
            return null;
        }
        ...
        if (splitLen >= bigBlock.getSize()) {
            ...
            InputStream inputStream = new ObsInputStream(obsClient, obsBucketName, outBlock.getObjName(), outBlock.getEndOffset(), originalEnd);
