...
public final class TableExpandUtil {
    ...
    /**
     * Split the table string(Usually contains names of some tables) to a List that is formated. example: table[0-32]
     * will be splitted into `table0`, `table1`, `table2`, ... ,`table32` in {@link List}
     *
            ...
            if (!matcher.matches()) {
                ...
                for (int k = Integer.valueOf(start); k <= Integer.valueOf(end); k++) {
                    ...
                    if (start.startsWith("0")) {
                        tableName =
                                schema + matcher.group(2).trim() + String.format("%0" + len + "d", k) + matcher.group(5)
                                        .trim();
                        splitTables.add(tableName);
                    } else {
    ...
    public static List<String> expandTableConf(DataBaseType dataBaseType, List<String> tables) {
        ...
        for (String table : tables) {
            List<String> splitTables = splitTables(dataBaseType, table);
