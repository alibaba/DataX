        ...
        public void privilegeValid(Configuration originalConfig, DataBaseType dataBaseType) {
            ...
            for (int i = 0, len = connections.size(); i < len; i++) {
                ...
                if (DBUtil.needCheckDeletePrivilege(originalConfig)) {
                    ...
                    if (!hasDeletePri) {
                        throw RdbmsException
                                .asDeletePriException(dataBaseType, originalConfig.getString(Key.USERNAME), jdbcUrl);
                    }
                }
            }
        }
    ...
    public static class Task {
            ...
            if (this.jdbcUrl.startsWith(Constant.OB10_SPLIT_STRING) && this.dataBaseType == DataBaseType.MySql) {
                ...
                if (ss.length != 3) {
                    throw DataXException
                            .asDataXException(DBUtilErrorCode.JDBC_OB10_ADDRESS_ERROR, "JDBC OB10格式错误，请联系askdatax");
                }
        ...
        private void calcWriteRecordSql() {
            if (!VALUE_HOLDER.equals(calcValueHolder(""))) {
                ...
                //ob10的处理
                if (dataBaseType != null && dataBaseType == DataBaseType.MySql && OriginalConfPretreatmentUtil
                        .isOB10(jdbcUrl)) {
                    forceUseUpdate = true;
                }
