...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        //ob10的处理
        if (url.startsWith(com.leehom.arch.datax.plugin.rdb2graph.rdbms.writer.Constant.OB10_SPLIT_STRING) && dataBaseType == DataBaseType.MySql) {
            String[] ss = url.split(com.leehom.arch.datax.plugin.rdb2graph.rdbms.writer.Constant.OB10_SPLIT_STRING_PATTERN);
            if (ss.length != 3) {
