...
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.reader.util.OriginalConfPretreatmentUtil;
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.reader.util.PreCheckTask;
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.reader.util.ReaderSplitUtil;
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.reader.util.SingleTableSplitUtil;
...
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.util.RdbmsException;
...
/**
 * @类名: CommonRdbms2GraphReader
 * @说明: rdb2graph读入器
 *
 * @author   leehom
 * @Date	 2022年4月29日 下午1:48:19
 * 修改记录：
 *
 * @see 	 
 */
public class CommonRdbms2GraphReader {
	...
	public static class Job {
		...
		public void init(Configuration originalConfig) {
			...
			//
        	String schemaUri = originalConfig.getString(Constant.RDB_SCHEMA_URI);
			try (InputStream is = ResourceLoaderUtil.getResourceStream(schemaUri);){
	    		byte[] bytes = ByteAndStreamUtils.StreamToBytes(is);
	    		rdbSchema = (DbSchema)ser.Unmarshal(bytes);
			} catch (Exception e) {
	        	throw DataXException.asDataXException(DBUtilErrorCode.ERROR_LOAD_RDBSCHEMA, e);
			} 
			// 原生配置预处理
			OriginalConfPretreatmentUtil.doPretreatment(originalConfig, rdbSchema);
	...
	public static class Task {
		...
		public void init(Configuration readerSliceConfig) {
			...
			//
			String phaseStr = readerSliceConfig.getString(Key.GRAPH_PHASE);
			this.phase = Phase.valueOf(phaseStr.toUpperCase());
			// ob10的处理
			if (this.jdbcUrl.startsWith(com.leehom.arch.datax.plugin.rdb2graph.rdbms.writer.Constant.OB10_SPLIT_STRING)
					&& this.dataBaseType == DataBaseType.MySql) {
				String[] ss = this.jdbcUrl
						.split(com.leehom.arch.datax.plugin.rdb2graph.rdbms.writer.Constant.OB10_SPLIT_STRING_PATTERN);
				if (ss.length != 3) {
		...
		public void startRead(Configuration readerSliceConfig, RecordSender recordSender,
			...
			try {
				...
				while (rs.next()) {
					...
					// 传输一条数据(记录)
					this.transportOneRecord(readerSliceConfig, recordSender, rs, metaData, columnNumber, mandatoryEncoding,
							taskPluginCollector);
		...
		// 传输一个记录
		protected Record transportOneRecord(Configuration readerSliceConfig, RecordSender recordSender, ResultSet rs, ResultSetMetaData metaData,
				int columnNumber, String mandatoryEncoding, TaskPluginCollector taskPluginCollector) {
			Record record = buildRecord(readerSliceConfig, recordSender, rs, metaData, columnNumber, mandatoryEncoding,
					taskPluginCollector);
			recordSender.sendToWriter(record);
			return record;
		}

		// 构建记录，resultset -> record
		// TableRecord RelRecord
		
		protected Record buildRecord(Configuration readerSliceConfig, RecordSender recordSender, ResultSet rs, ResultSetMetaData metaData,
				int columnNumber, String mandatoryEncoding, TaskPluginCollector taskPluginCollector) {
