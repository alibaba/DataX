...
import com.leehom.arch.datax.plugin.rdb2graph.rdbms.util.*;
...
/**
 * @类名: SingleTableSplitUtil
 * @说明: 单表分片
 *
 * @author   
 * @Date	 2022年4月29日 上午10:03:05
 * 修改记录：
 *
 * @see 	 
 */
public class SingleTableSplitUtil {
	...
	public static List<Configuration> splitSingleTable(Configuration configuration, int adviceNum) {
		...
		// if (Constant.SPLIT_MODE_RANDOMSAMPLE.equals(splitMode) && DATABASE_TYPE ==
		// DataBaseType.Oracle) {
		if (DATABASE_TYPE == DataBaseType.Oracle) {
			...
			if (isStringType) {
				rangeList = RdbmsRangeSplitWrap.splitAndWrap(String.valueOf(minMaxPK.getLeft()),
						String.valueOf(minMaxPK.getRight()), adviceNum, splitPkName, "'", DATABASE_TYPE);
			} else if (isLongType) {
		...
		if (null != rangeList && !rangeList.isEmpty()) {
			...
			Configuration tempConfig = configuration.clone();
			tempQuerySql = buildQuerySql(column, table, where) + (hasWhere ? " and " : " where ")
	...
	public static void precheckSplitPk(Connection conn, String pkRangeSQL, int fetchSize, String table,
			String username) {
	...
	/**
	 * 检测splitPk的配置是否正确。 configuration为null,
	 * 是precheck的逻辑，不需要回写PK_TYPE到configuration中
	 *
	 */
	 ...
	 */
	public static List<String> genSplitSqlForOracle(String splitPK, String table, String where,
		...
		// demo: Parameter rangeResult can not be null and its length can not <2.
		// detail:rangeResult=[24999930].
