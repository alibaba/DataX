...
ClickhouseWriter 插件实现了写入数据到 Clickhouse 主库的目的表的功能。在底层实现上， ClickhouseWriter 通过 JDBC 连接远程 Clickhouse 数据库，并执行相应的 insert into ... 的 sql 语句将数据写入 Clickhouse，内部会分批次提交入库，需要数据库本身采用 innodb 引擎。
...
ClickhouseWriter 通过 DataX 框架获取 Reader 生成的协议数据，根据你配置的 `writeMode` 生成
        ...
        "content": [
            {
                ...
                "writer": {
                    ...
                    "parameter": {
                        ...
                        "connection": [
                            {
                                "jdbcUrl": "jdbc:clickhouse://127.0.0.1:8123/datax",
                                "table": [
                                    "test"
                                ]
                            }
...
* **jdbcUrl**
	...
	* 描述：目的数据库的 JDBC 连接信息。作业运行时，DataX 会在你提供的 jdbcUrl 后面追加如下属性：yearIsDateType=false&zeroDateTimeBehavior=convertToNull&rewriteBatchedStatements=true
          ...
          注意：
               1. 在一个数据库上只能配置一个 jdbcUrl 值。这与 ClickhouseReader 支持多个备库探测不同，因为此处不支持同一个数据库存在多个主库的情况(双主导入数据情况)
               2. jdbcUrl按照Clickhouse官方规范，并可以填写连接附加控制信息，比如想指定连接编码为 gbk ，则在 jdbcUrl 后面追加属性 useUnicode=true&characterEncoding=gbk。具体请参看 Clickhouse官方文档或者咨询对应 DBA。
...
* **session**
	...
	* 描述: DataX在获取Clickhouse连接时，执行session指定的SQL语句，修改当前connection session属性
...
* **batchSize**
	...
	* 描述：一次性批量提交的记录数大小，该值可以极大减少DataX与Clickhouse的网络交互次数，并提升整体吞吐量。但是该值设置过大可能会造成DataX运行进程OOM情况。<br />
...
类似 ClickhouseReader ，目前 ClickhouseWriter 支持大部分 Clickhouse 类型，但也存在部分个别类型没有支持的情况，请注意检查你的类型。

下面列出 ClickhouseWriter 针对 Clickhouse 类型转换列表:
...
* 执行DataX的机器参数为:
    1. cpu: 4核 Intel(R) Xeon(R) CPU E5-2682 v4 @ 2.50GHz
    2. mem: 32GB
    3. net: 百兆网卡
