...
public class WriterModelParser {
    ...
    public static OTSAttrColumn parseOTSAttrColumn(Map<String, Object> column) {
        if (column.containsKey(OTSConst.NAME) && column.containsKey(OTSConst.TYPE)
                && column.containsKey(OTSConst.SEQUENCE) && column.size() == 3) {
            Object type = column.get(OTSConst.TYPE);
            Object name = column.get(OTSConst.NAME);
            Object sequence = column.get(OTSConst.SEQUENCE);
            if (type instanceof String && name instanceof String && sequence instanceof Integer) {
                ...
                return new OTSAttrColumn(nameStr, parseColumnType(typeStr), sequenceInt);
            } else {
