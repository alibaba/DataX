...
public class ESClient {
    ...
    public SearchResult exe(Search search) throws IOException {
        return jestClient.execute(search);
    }

    public JestResult exeScroll(SearchScroll searchScroll) throws IOException {
        return jestClient.execute(searchScroll);
    }

    public void clearScrollIds(String scrollId){
        ClearScroll.Builder build = new ClearScroll.Builder().addScrollId(scrollId);
        ClearScroll clearScroll =build.build();
        try {
            jestClient.execute(clearScroll);
        } catch (IOException e) {
            log.error("ESJestClient Clean ScrollIds Exception...", e);
        }
    }
}
