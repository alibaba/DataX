        ...
        "content": [
            {
                ...
                "writer": {
                    ...
                    "parameter": {
                        ...
                        "connection": [
                            {
                                "jdbcUrl": "jdbc:xugu://127.0.0.1:5138/datax",
                                "table": [
                                    "test"
                                ]
                            }
...
* **jdbcUrl**
	...
	* 描述：目的数据库的 JDBC 连接信息 ,jdbcUrl必须包含在connection配置单元中。
               ...
               注意：1、在一个数据库上只能配置一个 jdbcUrl 值。这与 XuguReader 支持多个备库探测不同，因为此处不支持同一个数据库存在多个主库的情况(双主导入数据情况)
                    2、jdbcUrl按照Xugu官方规范，并可以填写连接附加控制信息，具体请参看 Xugu官方文档或者咨询对应 DBA。
...
* **session**
	...
	* 描述: DataX在获取Xugu连接时，执行session指定的SQL语句，修改当前connection session属性
...
建表语句：
	...
	CREATE TABLE `datax_xuguwriter_perf_00` (
