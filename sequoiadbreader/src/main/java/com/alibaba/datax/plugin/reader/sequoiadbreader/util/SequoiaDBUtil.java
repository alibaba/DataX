...
public class SequoiaDBUtil {
    ...
    public static Sequoiadb initAuthenticationSDBClient(Configuration conf, String userName, String password) {
        ...
        if(!isHostPortPattern(addressList)) {
            throw DataXException.asDataXException(SequoiaDBReaderErrorCode.ILLEGAL_VALUE,"不合法参数");
        }
        ...
        try {
            ...
            if(e.getErrorCode() == -6 ) {
                ...
                throw DataXException.asDataXException(SequoiaDBReaderErrorCode.NETWORK_ERROR, "网络错误");
            } else if(e.getErrorCode() == -79 ) {
                throw DataXException.asDataXException(SequoiaDBReaderErrorCode.SDB_NET_CANNOT_CONNECT, "网络连接失败");
            } else if(e.getErrorCode() == -179) {
                throw DataXException.asDataXException(SequoiaDBReaderErrorCode.SDB_AUTH_AUTHORITY_FORBIDDEN, "用户或密码错误");
            } else {
                throw DataXException.asDataXException(SequoiaDBReaderErrorCode.UNEXCEPT_EXCEPTION, "未知异常");
            }
        }
    }
